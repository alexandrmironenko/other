...
gulp.task('css', function(){ // Создаем таск Sass
	return gulp.src('sass/**/*.sass') // Берем источник
		.pipe(sass()) // Преобразуем Sass в CSS посредством gulp-sass
		.pipe(autoprefixer(['last 15 versions', '> 1%', 'ie 8', 'ie 7'], { cascade: true })) // Создаем префиксы
		.pipe(gulp.dest('custom')) // Выгружаем результата в папку app/css
		.pipe(browserSync.reload({stream: true})) // Обновляем CSS на странице при изменении
});
...
// 	gulp.watch('css/**/*.css', ['css']); // Наблюдение за sass файлами в папке sass
// 	gulp.watch('custom/**/*.css', ['css']); // Наблюдение за sass файлами в папке sass
...
gulp.task('watch', ['browser-sync', 'css'], function() {
	gulp.watch('sass/**/*.sass', ['css']); // Наблюдение за sass файлами в папке sass
	gulp.watch('css/**/*.css', ['css']); // Наблюдение за sass файлами в папке sass
	gulp.watch('custom/**/*.css', ['css']); // Наблюдение за sass файлами в папке sass
	// gulp.watch('app/*.html', browserSync.reload); // Наблюдение за HTML файлами в корне проекта
	// gulp.watch('app/**/*.php', browserSync.reload);
